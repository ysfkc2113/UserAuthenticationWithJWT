...
{
    public class ClubUserManagerClubLeader:IClubUserServiceClubLeader
    {
        ...
        public ClubUserManagerClubLeader(IRepositoryManager manager, IMapper mapper, UserManager<User> userManager)
        {
            _manager = manager;
            _mapper = mapper;
        ...
        public async Task<(IEnumerable<AdminClubUserDtoRelations> club_Users, MetaData metaData)> GetAllUsersByClubIdForClubManagerAsync(ClubUserParameters clubUserParameters, HttpContext httpContext, bool trackChanges)
        {
        ...
        public async Task ChangeApprovedForClubManagerAsync(int id, HttpContext httpContext, bool trackChanges)
        {
            var userName_club = await GetUserNameByHttpContextAsync(httpContext);
        ...
        public async Task ChangeClubRoleForClubManagerAsync(AdminClubUserDtoChangeRole adminClubUserDtoChangeRole, HttpContext httpContext, bool trackChanges)
        {
            var userName_club = await GetUserNameByHttpContextAsync(httpContext);
            ...
            {
                throw new Exception("Bu kişi sizsiniz.Kendi rolünüzü değiştiremezsiniz.");

            }
            // rütbe yükseltme
            if (clubuser.role_in_club == "Club Manager")
                throw new Exception("Bu kulübün zaten bir yöneticisi var.Birden fazla yönetici rolü olamaz.");
